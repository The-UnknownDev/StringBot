...
ERROR_MESSAGE = "Oops! An exception occurred! \n\n**Error** : {} " \
            "\n\nPlease Report to @br29siwan if there is an error " \
...
async def generate_session(bot, msg, telethon=False):
    await msg.reply(
        f'Getting Started with Session Generation {"Telethon" if telethon else "Pyrogram"}s....'
    )
    ...
    text = "**{} STRING SESSION** \n\n`{}` \n\nGenerated by @sessiongenro_bot".format("TELETHON" if telethon else "PYROGRAM", string_session)
    await client.send_message("me", text)
    await client.disconnect()
    await phone_code_msg.reply("Successfully fetching session string {}. \n\nPlease check on Saved Messages! \n\nBy @sessiongenro_bot".format("telethon" if telethon else "pyrogram"))
